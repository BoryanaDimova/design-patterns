	...
	private void checkMessageForForbiddenWord(String message, User sender) {
		...
		while (matcher.find()) {
			if ("cat".equals(matcher.group().toLowerCase())) {
				...
				System.out.println("The Bot with nickname ~" + this.nickname
						+ " removed the user with nickname ~" + sender.nickname
						+ "~ for using the forbidden word --'cat'-- in his message '" + message + "'.");
